...
public class PinchImageView extends AppCompatImageView {
    ...
    private final String TAG = PinchImageView.class.getSimpleName();
     ...
     * 图片初始化状态
     */
    public static final int IMG_STATUS_INIT = 21;
    /**
     * 图片放大状态
     */
    public static final int IMG_STATUS_ENLARGE = 22;
    /**
     * 图片放大滚动到左边边缘状态
     */
    public static final int IMG_STATUS_ENLARGE_EDGE_LEFT = 23;
    /**
     * 图片放大滚动到右边边缘状态
     */
    public static final int IMG_STATUS_ENLARGE_EDGE_RIGHT = 24;

    /**
     * 图片的初始大小
     */
    private RectF imgInitBound;

    public RectF getImgInitBound() {
        return this.imgInitBound;
    }

    /**
     * 图片的当前状态
     */
    private int currentImgStatus = IMG_STATUS_INIT;

    public int getCurrentImgStatus() {
        return this.currentImgStatus;
    }

    private OuterTouchOverListener touchOverListener;
    ...
    public interface OuterTouchOverListener {
        void OuterTouchOver(PinchImageView pinchImageView);
    }

    public void addOuterTouchOverListener(OuterTouchOverListener listener) {
        this.touchOverListener = listener;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 获取图片初始大小
        if (imgInitBound == null) {
            imgInitBound = getImageBound(null);
            Log.i(TAG, "image init size:" + imgInitBound.toString());
        }
     ...
     * @return
     */
    private boolean checkRectF(RectF rectf, RectF mRectF) {
        return Math.abs(rectf.left - mRectF.left) < 1
                && Math.abs(rectf.right - mRectF.right) < 1
                && Math.abs(rectf.top - mRectF.top) < 1
                && Math.abs(rectf.right - mRectF.right) < 1;
    }

    private void addTouchOverEvent() {
        if (imgInitBound == null) {
            return;
        }
        // 计算当前图片状态
        // 允许一些误差存在，避免太精确导致有些功能失效
        RectF rectF = getImageBound(null);
        if (checkRectF(rectF, imgInitBound)) {
            currentImgStatus = IMG_STATUS_INIT;
        } else {
            currentImgStatus = IMG_STATUS_ENLARGE;
            if (Math.abs(rectF.left) < 1) {
                currentImgStatus = IMG_STATUS_ENLARGE_EDGE_LEFT;
            }
            if (Math.abs(rectF.right - (imgInitBound.right + imgInitBound.left)) < 1) {
                currentImgStatus = IMG_STATUS_ENLARGE_EDGE_RIGHT;
            }
        }
        if (touchOverListener != null) {
            touchOverListener.OuterTouchOver(this);
        }
    }
    ...
    private void doubleTap(float x, float y) {
        ...
        startScaleAnimator(mScaleAnimator);
        //清理临时变量
     ...
     * @param scaleAnimator
     */
    private void startScaleAnimator(ScaleAnimator scaleAnimator) {
    ...
    private void fling(float vx, float vy) {
        ...
        startFlingAnimator(mFlingAnimator);
    }

    private void startFlingAnimator(FlingAnimator flingAnimation) {
        if (flingAnimation == null)
            return;

        flingAnimation.start();
        flingAnimation.addListener(new Animator.AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animation) {
